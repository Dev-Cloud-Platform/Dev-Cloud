...
def check_vms_2(lv_vms, db_vms, n_images):
    db_vms = ['vm-%d-%d' % (vm['id'], vm['user_id']) for vm in db_vms if
              vm_states_reversed[vm['state']] == 'running' or vm_states_reversed[vm['state']] == 'running ctx']
...
db_all_vms = [
    {'id': vm.id, 'user_id': vm.user_id, 'state': vm.state, 'vnc_port': vm.vnc_port, 'node': vm.node.ssh_string} for vm
    in VM.objects.all()]
for node in [{'id': n.id, 'addr': n.ssh_string} for n in Node.objects.filter(state=1).all()]:
    ...
    db_vms = [{'id': vm.id, 'user_id': vm.user_id, 'state': vm.state, 'vnc_port': vm.vnc_port} for vm in
              VM.objects.filter(node_id=node['id']).all()]
