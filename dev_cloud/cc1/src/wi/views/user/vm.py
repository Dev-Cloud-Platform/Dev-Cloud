...
class CreateVMWizard(CustomWizardView):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        if self.steps.current == '0':
            ...
            rest_data = prep_data(
                {'image': ('user/system_image/get_by_id/', {'system_image_id': form_cleaned_data['image_id']}),
                 }, self.request.session)

            context.update({'steps_desc': [
                rest_data['image']['name'] if len(rest_data['image']['name']) <= 15 else rest_data['image']['name'][
                                                                                         :15] + '...', _('Hardware'),
                _('Optional resources'), _('Summary')]})
            ...
            template = utils.get_dict_from_list(rest_data['templates'], form_cleaned_data['template_id'],
                                                key='template_id')
            context.update({'steps_desc': [
                rest_data['image']['name'] if len(rest_data['image']['name']) <= 15 else rest_data['image']['name'][
                                                                                         :15] + '...',
                str(template['cpu']) + '/' + str(template['memory']), _('Optional resources'), _('Summary')]})
            ...
            summary_data = {'summary_image': rest_data['image'],
                            'summary_template': utils.get_dict_from_list(rest_data['templates'],
                                                                         form_cleaned_data['template_id'],
                                                                         key='template_id'),
                            'summary_ip': utils.get_dict_from_list(rest_data['ips'], form_cleaned_data['public_ip_id'],
                                                                   key='public_ip_id'),
                            'summary_disks': utils.get_dicts_from_list(rest_data['disks'],
                                                                       form_cleaned_data['disk_list'],
                                                                       key='storage_image_id'),
                            'summary_iso': utils.get_dicts_from_list(rest_data['iso'], form_cleaned_data['iso_list'],
                                                                     key='iso_image_id'),
...
def vms_ajax_get_table(request):
    ...
    if request.method == 'GET':
        ...
        for item in rest_data:
            ...
            item['cpuLoadColor'] = "#%02x%02x%02x" % tuple(
                i * 255 for i in hsv_to_rgb(float(item['cpuLoadPercent']) / 300, 1.0, 0.8))
            item['pub_ip'] = []
...
def vms_ajax_revoke_ip(request, vm_id, template_name='generic/form.html', form_class=RevokeIPForm):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            prep_data(('user/public_ip/unassign/', {'public_ip_id': form.cleaned_data['public_ip_id']}),
                      request.session)
