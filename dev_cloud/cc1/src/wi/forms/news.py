...
class NewsForm(forms.Form):
    ...
    content = forms.CharField(widget=forms.Textarea(attrs=dict(attrs_dict, maxlength=512)),
                              label=_('Content'),
                              help_text=_(
                                  'News content - may include Markdown markup language, e.g., *text* for italics, **text** for bold text, * item for unordered list, 1. item for ordered list, [link name](http://address) for anchor, `code` for preformated code.'))
