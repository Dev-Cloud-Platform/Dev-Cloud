...
class AccountDataEdit(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['default_cluster_id'] = forms.ChoiceField(choices=parsing.parse_cm_list(rest_data),
                                                              label=_('Default CM'))
...
class CMAuthenticationForm(forms.Form):
    ...
    def clean(self):
        ...
        if password and not cm_authenticate(self.request.session['user'], password, cm):
            raise forms.ValidationError(
                _('Please enter a correct password. Note that password field is case-sensitive.'))
