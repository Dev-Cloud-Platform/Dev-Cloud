...
from wi.utils.states import stat_names_reversed, stat_short_names_reversed, stat_resolutions_reversed, \
...
class My2CheckboxSelectMultiple(forms.CheckboxSelectMultiple):
    def render(self, name, value, attrs=None, choices=()):
        ...
        for i, (option_value, option_label) in enumerate(chain(self.choices, choices)):
            ...
            output.append(u'<li class="stats ' + stat_short_names_reversed[
                int(option_value)] + '"><label%s>%s %s</label></li>' % (label_for, rendered_cb, option_label))
        output.append(u'</ul>')
        return mark_safe(u'\n'.join(output))
