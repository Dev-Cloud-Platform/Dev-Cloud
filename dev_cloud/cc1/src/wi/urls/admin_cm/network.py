...
network_patterns = patterns('wi.views.admin_cm.network',
                            ...
                            url(r'^ajax/cm/networks_get_table/(?P<user_id>\d+)/$', 'cma_networks_ajax_get_table',
                                name='cma_networks_ajax_get_table'),
                            url(r'^ajax/cm/network_details/(?P<network_id>\d+)/$', 'cma_networks_ajax_network_details',
                            ...
                            url(r'^ajax/remove_network/(?P<id1>\d+)/$', admin_cm_permission(simple_generic_id),
                                {'template_name': 'generic/simple.html',
                                 'success_msg': (
                                 lambda desc: _('You have successfully released network <b>%(desc)s</b>.') % {
                                     'desc': desc}),
                                 'ask_msg': (
                                 lambda desc: _('Do you want to release network <b>%(desc)s</b>?') % {'desc': desc}),
                                 'request_url': 'admin_cm/network/delete_user_network/',
                                 'id_key': 'network_id', },
                                name='cma_ajax_remove_network'),

                            url(r'^pools/$', admin_cm_permission(direct_to_template),
                                {'template_name': 'admin_cm/pools.html'}, name='cma_pools'),
                            ...
                            url(r'^ajax/add_pool/$', admin_cm_permission(form_generic),
                                {'template_name': 'generic/form.html',
                                 'success_msg': (
                                 lambda desc, data: _('You have successfully added a pool.') % {'desc': desc}),
                                 'confirmation': _('Add'),
                            ...
                            url(r'^ajax/delete_pool/(?P<id1>\d+)/$', admin_cm_permission(simple_generic_id),
                                {'template_name': 'generic/simple.html',
                                 'success_msg': (
                                 lambda desc: _('You have successfully deleted pool <b>%(desc)s</b>.') % {
                                     'desc': desc}),
                                 'ask_msg': (
                                 lambda desc: _('Do you want to delete pool <b>%(desc)s</b>?') % {'desc': desc}),
                            ...
                            url(r'^ajax/lock_pool/(?P<id1>\d+)/$', admin_cm_permission(simple_generic_id),
                                {'template_name': 'generic/simple.html',
                                 'success_msg': (
                                 lambda desc: _('You have successfully locked pool <b>%(desc)s</b>.') % {'desc': desc}),
                                 'ask_msg': (
                                 lambda desc: _('Do you want to lock pool <b>%(desc)s</b>?') % {'desc': desc}),
                            ...
                            url(r'^ajax/unlock_pool/(?P<id1>\d+)/$', admin_cm_permission(simple_generic_id),
                                {'template_name': 'generic/simple.html',
                                 'success_msg': (
                                 lambda desc: _('You have successfully unlocked pool <b>%(desc)s</b>.') % {
                                     'desc': desc}),
                                 'ask_msg': (
                                 lambda desc: _('Do you want to unlock pool <b>%(desc)s</b>?') % {'desc': desc}),
                                 'request_url': 'admin_cm/network/unlock/',
                                 'id_key': 'pool_id', },
                                name='cma_ajax_unlock_pool'),

                            url(r'^publicips/$', admin_cm_permission(direct_to_template),
                                {'template_name': 'admin_cm/publicips.html'}, name='cma_publicips'),
                            url(r'^ajax/publicips_table/$', admin_cm_permission(get_list_generic),
                                {'request_url': 'admin_cm/public_ip/get_list/'},
                                name='cma_ajax_get_publicips_table'),
                            url(r'^ajax/add_publicip/$', admin_cm_permission(form_generic),
                                {'template_name': 'generic/form.html',
                                 'success_msg': (
                                 lambda desc, data: _('You have successfully added public IPs.') % {'desc': desc}),
                                 'confirmation': _('Add'),
                            ...
                            url(r'^ajax/cm/publicip_delete/$', admin_cm_permission(generic_multiple_id),
                                {'template_name': 'generic/simple.html',
                                 ...
                                 'ask_msg': (
                                 lambda desc, count: ungettext('Do you want to delete public IP <b>%(desc)s</b>?',
                                                               'Do you want to delete %(count)d public IPs <b>%(desc)s</b>?',
                                ...
                                {'template_name': 'generic/simple.html',
                                 'success_msg': (lambda desc, count: ungettext(
                                     ...
                                     'You have successfully released %(count)d pulic IPs (<b>%(desc)s</b>).', count) % {
                                                                         'desc': desc, 'count': count}),
                                 'ask_msg': (
                                 lambda desc, count: ungettext('Do you want to release public IP <b>%(desc)s</b>?',
                                                               'Do you want to release %(count)d public IPs <b>%(desc)s</b>?',
                                                               count) % {'desc': desc, 'count': count}),
