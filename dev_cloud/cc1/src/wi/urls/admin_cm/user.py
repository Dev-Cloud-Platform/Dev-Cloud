...
user_patterns = patterns('wi.views.admin_cm.user',
                         url(r'^users/$', admin_cm_permission(direct_to_template),
                             {'template_name': 'admin_cm/users.html'}, name='cma_users'),
                         url(r'^user_account/(?P<user_id>\d+)/$', 'cma_user_account', name='cma_user_account'),
                         url(r'^ajax/get_user_data/(?P<user_id>\d+)/$', 'cma_ajax_get_user_data',
                             name='cma_ajax_get_user_data'),
                         ...
                         url(r'^ajax/edit_quota/(?P<id1>\d+)/$', admin_cm_permission(form_generic_id),
                             {'template_name': 'generic/form.html',
                              'success_msg': (
                              lambda desc, data: _('You have successfully changed the user\'s quota.') % {
                                  'desc': desc}),
                              'confirmation': _('Save'),
                             ...
                             {'template_name': 'generic/form.html',
                              'success_msg': (lambda desc, data: _(
                                  'You have successfully promoted user <b>%(desc)s</b> to CM administrator.') % {
                                                                     'desc': desc}),
                              'ask_msg': (
                              lambda desc: _('Do you want to promote user <b>%(desc)s</b> to CM administrator?') % {
                                  'desc': desc}),
                              'request_url_post': 'admin_cm/admin/add/',
                              'id_key': 'user_id',
                              'form_class': PasswordForm},
                             name='cma_ajax_set_admin'),
                         url(r'^ajax/change_cm_password/$', 'cma_ajax_change_cm_password',
                             name='cma_ajax_change_cm_password'),
                             ...
                             {'template_name': 'generic/simple.html',
                              'success_msg': (lambda desc: _(
                                  'You have successfully demoted administrator <b>%(desc)s</b> to regular user.') % {
                                                               'desc': desc}),
                              'ask_msg': (
                              lambda desc: _('Do you want to demote administrator <b>%(desc)s</b> to simple user?') % {
                                  'desc': desc}),
