...
def simple_generic(request, template_name=None, success_msg=lambda x: '', ask_msg=lambda x: '', confirmation=_('Yes'),
                   request_url=None, param=None):
    """
...
@ajax_request
@csrf_protect
def simple_generic_twoid(request, id1, id2, template_name=None, success_msg=lambda desc: _('Success') % {'desc': desc},
                         ask_msg=lambda x: '',
    ...
    if template_name is not None:
        return messages_ajax.success(
            render_to_string(template_name, {'text': ask_msg(force_escape(request.REQUEST.get('desc'))),
                                             'confirmation': confirmation, 'id': id1},
...
def form_generic_id(request, id1, template_name=None, form_class=None,
    ...
    return messages_ajax.success(render_to_string(template_name, {'form': form,
                                                                  'text': ask_msg(
                                                                      force_escape(request.REQUEST.get('desc'))),
...
def generic_multiple_id(request, template_name=None, success_msg=lambda x: _('Success'),
                        ask_msg=lambda x, y: _('Do you want to?'), confirmation=_('Yes'), request_url=None,
                        id_key=None):
    """
...
class CustomWizardView(CookieWizardView):
    ...
    def post(self, *args, **kwargs):
        ...
        if self.request.POST[self.wizard_name + '-current_step'] != '0' and self.request.COOKIES.get(
                        'wizard_' + self.wizard_name).find("\"step_data\":{}") != -1:
