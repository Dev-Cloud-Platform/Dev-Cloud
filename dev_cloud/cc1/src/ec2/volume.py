        ...
        # and if they were
            ...
            try:
                for volume_id in volume_ids:
                    clm_volumes.append(
                        self.cluster_manager.user.storage_image.get_by_id({'storage_image_id': volume_id}))
            except CLMException, error:
    ...
    def _execute(self):
        ...
        if instance_id_ec2:
            ...
            try:
                ...
                self.cluster_manager.user.storage_image.detach(
                    {'storage_image_id': volume_id, 'vm_id': volume_dict['vm_id']})
            except CLMException, error:
                if error.status == 'storage_image_detach':
                    raise UndefinedError
                if error.status == 'image_attached':
