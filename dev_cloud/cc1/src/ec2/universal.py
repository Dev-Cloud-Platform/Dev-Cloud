            ...
            if filters.get('resource-id'):
                for filter in filters['resource-id']:
                    if filter.startswith('vol-') and (
                        not filters.get('resource-type') or filters.get('resource-type') == 'volume'):
                        tags.append(get_volume_name_tag(self.cluster_manager, filter[4:]))
                    elif filter.startswith('i-') and (
                        not filters.get('resource-type') or filters.get('resource-type') == 'instance'):
        ...
        for resource in resourceIds:
            ...
            if resource.startswith('i-'):
                ...
                self.cluster_manager.user.vm.edit(
                    {'vm_id': instance_id, 'name': 'i-' + instance_id, 'description': instance['description']})
