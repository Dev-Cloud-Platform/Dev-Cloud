...
class User(models.Model):
    ...
    def used_cpu(self):
        ...
        c = self.vm_set.filter(state__in=[vm_states['running'], vm_states['running ctx'], vm_states['init']]).aggregate(
            cpu_sum=Sum('template__cpu'))
    ...
    def used_memory(self):
        ...
        m = self.vm_set.filter(state__in=[vm_states['running'], vm_states['running ctx'], vm_states['init']]).aggregate(
            memory_sum=Sum('template__memory'))
    ...
    def used_points(self):
        ...
        # TODO:TEST
        vms = self.vm_set.exclude(
            state__in=[vm_states['failed'], vm_states['saving failed'], vm_states['erased']]).exclude(
            stop_time__lte=start)
