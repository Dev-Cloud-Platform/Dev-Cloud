        ...
        try:
            ...
            try:
                while retry > 0:
                    ...
                    log.debug(user_id, "Checked command status: %s, %s, %s" % (
                    cmd.state, command_states['finished'], bool(cmd.state == command_states['finished'])))
    ...
    def register_head(vm):
        ...
        if vm.farm.state == farm_states['init_head']:
            ...
            try:
                ...
                ssh_username = 'root'
                ssh_key = r
                log.debug(vm.user_id,
                          'appended %d vms to farm [id:%d]' % (vm.farm.vms.count() - 1, vm.id))  # excluding head
