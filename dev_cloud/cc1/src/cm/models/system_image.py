...
class SystemImage(Image):
    ...
    def dict(self):
        ...
        d['vms'] = list(self.vm_set.filter(state__in=[vm_states['running'], vm_states['running_ctx']]).values_list('id',
                                                                                                                   flat=True)) or []
    ...
    def has_access(self, user_id, groups=None):
        ...
        if self.user.id != user_id:
            ...
            elif self.access == image_access['group'] and (
                (groups is None) or (not (self.systemimagegroup_set.filter(group_id__in=groups).exists()))):
                raise CMException('image_permission')
        return True
