...
class VM(models.Model):
    ...
    def create(user, name, description, image_id, template_id, public_ip_id, iso_list, disk_list, vnc, groups,
        ...
        for i in range(count):
            ...
            node = Node.get_free_node(head_template, image, node_id) if farm and i == 0 else Node.get_free_node(
            ...
            used_ports = VM.objects.exclude(state__in=[vm_states['closed'], vm_states['erased']]).values_list(
                'vnc_port', flat=True)
