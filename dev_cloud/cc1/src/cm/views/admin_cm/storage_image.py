...
def download(caller_id, description, name, path, disk_dev, disk_controller):
    ...
    image = StorageImage.create(name=name, description=description, user=user, disk_dev=disk_dev,
...
def convert_to_system_image(caller_id, storage_image_id):
    ...
    system_image = SystemImage.create(name=image.name, description=image.description, user=image.user,
