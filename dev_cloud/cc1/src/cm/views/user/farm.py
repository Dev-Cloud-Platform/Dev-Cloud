...
@user_log(log=True)
def create(caller_id, name, description, image_id, head_template_id, worker_template_id, public_ip_id, iso_list,
           disk_list, vnc, groups, count):
    """
    ...
    vms = VM.create(user, name=name, description=description, image_id=image_id, template_id=worker_template_id,
                    head_template_id=head_template_id, public_ip_id=public_ip_id, iso_list=iso_list,
                    disk_list=disk_list,
...
def get_list(caller_id):
    ...
    """
    farms = [farm.dict for farm in
             Farm.objects.exclude(state=farm_states['closed']).filter(user__id__exact=caller_id).order_by('-id')]
